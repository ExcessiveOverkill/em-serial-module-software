...
Mode::Mode(logging* logs, estop_io* io, device_struct** comm_vars){
    this->logs = logs;
    this->EstopIO = io;
...
void Mode::run_checks(void){
    ...
    if(EstopIO->inputs.brake_1 && !EstopIO->outputs.brake_1){
        logs->log_persistent_active(checks_messages::brake_1_powered_when_off);
    }
    else{
        logs->log_persistent_inactive(checks_messages::brake_1_powered_when_off);
    }
    if(EstopIO->inputs.brake_2 && !EstopIO->outputs.brake_2){
        logs->log_persistent_active(checks_messages::brake_2_powered_when_off);
    }
    else{
        logs->log_persistent_inactive(checks_messages::brake_2_powered_when_off);
    }
    if(EstopIO->inputs.brake_3 && !EstopIO->outputs.brake_3){
        logs->log_persistent_active(checks_messages::brake_3_powered_when_off);
    }
    else{
        logs->log_persistent_inactive(checks_messages::brake_3_powered_when_off);
    }
    if(EstopIO->inputs.brake_4 && !EstopIO->outputs.brake_4){
        logs->log_persistent_active(checks_messages::brake_4_powered_when_off);
    }
    else{
        logs->log_persistent_inactive(checks_messages::brake_4_powered_when_off);
    }
    if(EstopIO->inputs.brake_5 && !EstopIO->outputs.brake_5){
        logs->log_persistent_active(checks_messages::brake_5_powered_when_off);
    }
    else{
        logs->log_persistent_inactive(checks_messages::brake_5_powered_when_off);
    }
    if(EstopIO->inputs.brake_6 && !EstopIO->outputs.brake_6){
        logs->log_persistent_active(checks_messages::brake_6_powered_when_off);
    }
    else{
        logs->log_persistent_inactive(checks_messages::brake_6_powered_when_off);
    }

    // stuck off brake outputs
    if(!EstopIO->inputs.brake_1 && EstopIO->outputs.brake_1){
        logs->log_persistent_active(checks_messages::brake_1_not_powered_when_on);
    }
    else{
        logs->log_persistent_inactive(checks_messages::brake_1_not_powered_when_on);
    }
    if(!EstopIO->inputs.brake_2 && EstopIO->outputs.brake_2){
        logs->log_persistent_active(checks_messages::brake_2_not_powered_when_on);
    }
    else{
        logs->log_persistent_inactive(checks_messages::brake_2_not_powered_when_on);
    }
    if(!EstopIO->inputs.brake_3 && EstopIO->outputs.brake_3){
        logs->log_persistent_active(checks_messages::brake_3_not_powered_when_on);
    }
    else{
        logs->log_persistent_inactive(checks_messages::brake_3_not_powered_when_on);
    }
    if(!EstopIO->inputs.brake_4 && EstopIO->outputs.brake_4){
        logs->log_persistent_active(checks_messages::brake_4_not_powered_when_on);
    }
    else{
        logs->log_persistent_inactive(checks_messages::brake_4_not_powered_when_on);
    }
    if(!EstopIO->inputs.brake_5 && EstopIO->outputs.brake_5){
        logs->log_persistent_active(checks_messages::brake_5_not_powered_when_on);
    }
    else{
        logs->log_persistent_inactive(checks_messages::brake_5_not_powered_when_on);
    }
    if(!EstopIO->inputs.brake_6 && EstopIO->outputs.brake_6){
        logs->log_persistent_active(checks_messages::brake_6_not_powered_when_on);
    }
    else{
        logs->log_persistent_inactive(checks_messages::brake_6_not_powered_when_on);
    }
    ...
    if((EstopIO->inputs.brake_A && !EstopIO->outputs.chain_relay) || (EstopIO->inputs.brake_B && !EstopIO->outputs.chain_relay)){
        logs->log_persistent_active(checks_messages::brake_main_relay_stuck_on);
    }
    else{
        logs->log_persistent_inactive(checks_messages::brake_main_relay_stuck_on);
    }
    ...
    if(!(EstopIO->inputs.brake_A && EstopIO->inputs.brake_B) && EstopIO->outputs.chain_relay && EstopIO->inputs.chain_A && EstopIO->inputs.chain_B){
        logs->log_persistent_active(checks_messages::brake_supply_missing);
    }
    else{
        logs->log_persistent_inactive(checks_messages::brake_supply_missing);
    }

    // estop chain mismatch
    if(EstopIO->inputs.chain_A != EstopIO->inputs.chain_B){
        logs->log_persistent_active(checks_messages::estop_chain_mismatch);
    }
    else{
        logs->log_persistent_inactive(checks_messages::estop_chain_mismatch);
    }
    ...
    if((EstopIO->inputs.chain_A_relay || EstopIO->inputs.chain_B_relay) && !EstopIO->outputs.chain_relay && !(*comm_vars)->STO_only){
        logs->log_persistent_active(checks_messages::chain_relay_stuck_on);
    }
    else{
        logs->log_persistent_inactive(checks_messages::chain_relay_stuck_on);
    }
    ...
    if(EstopIO->outputs.chain_relay && (EstopIO->inputs.chain_A && EstopIO->inputs.chain_B) && (!EstopIO->inputs.chain_A_relay || !EstopIO->inputs.chain_B_relay)){
        logs->log_persistent_active(checks_messages::contactor_supply_missing);
    }
    else{
        logs->log_persistent_inactive(checks_messages::contactor_supply_missing);
    }

    // contactors powered when off
    if(EstopIO->inputs.isolation_contactor_int && !EstopIO->outputs.isolation_contactor){
        logs->log_persistent_active(checks_messages::isolation_contactor_powered_when_off);
    }
    else{
        logs->log_persistent_inactive(checks_messages::isolation_contactor_powered_when_off);
    }
    if(EstopIO->inputs.main_contactor_int && !EstopIO->outputs.main_contactor){
        logs->log_persistent_active(checks_messages::main_contactor_powered_when_off);
    }
    else{
        logs->log_persistent_inactive(checks_messages::main_contactor_powered_when_off);
    }
    if(EstopIO->inputs.precharge_contactor_int && !EstopIO->outputs.precharge_contactor){
        logs->log_persistent_active(checks_messages::precharge_contactor_powered_when_off);
    }
    else{
        logs->log_persistent_inactive(checks_messages::precharge_contactor_powered_when_off);
    }

    // contactors not powered when on
    if(!EstopIO->inputs.isolation_contactor_int && EstopIO->outputs.isolation_contactor){
        logs->log_persistent_active(checks_messages::isolation_contactor_not_powered_when_on);
    }
    else{
        logs->log_persistent_inactive(checks_messages::isolation_contactor_not_powered_when_on);
    }
    if(!EstopIO->inputs.main_contactor_int && EstopIO->outputs.main_contactor){
        logs->log_persistent_active(checks_messages::main_contactor_not_powered_when_on);
    }
    else{
        logs->log_persistent_inactive(checks_messages::main_contactor_not_powered_when_on);
    }
    if(!EstopIO->inputs.precharge_contactor_int && EstopIO->outputs.precharge_contactor){
        logs->log_persistent_active(checks_messages::precharge_contactor_not_powered_when_on);
    }
    else{
        logs->log_persistent_inactive(checks_messages::precharge_contactor_not_powered_when_on);
    }

    // external contactors closed when off
    if(EstopIO->inputs.isolation_contactor_ext && !EstopIO->outputs.isolation_contactor){
        logs->log_persistent_active(checks_messages::isolation_contactor_feedback_preset_when_off);
    }
    else{
        logs->log_persistent_inactive(checks_messages::isolation_contactor_feedback_preset_when_off);
    }
    if(EstopIO->inputs.main_contactor_ext && !EstopIO->outputs.main_contactor){
        logs->log_persistent_active(checks_messages::main_contactor_feedback_preset_when_off);
    }
    else{
        logs->log_persistent_inactive(checks_messages::main_contactor_feedback_preset_when_off);
    }
    if(EstopIO->inputs.precharge_contactor_ext && !EstopIO->outputs.precharge_contactor){
        logs->log_persistent_active(checks_messages::precharge_contactor_feedback_preset_when_off);
    }
    else{
        logs->log_persistent_inactive(checks_messages::precharge_contactor_feedback_preset_when_off);
    }

    // external contactors open when on
    if(!EstopIO->inputs.isolation_contactor_ext && EstopIO->outputs.isolation_contactor){
        logs->log_persistent_active(checks_messages::isolation_contactor_feedback_missing_when_on);
    }
    else{
        logs->log_persistent_inactive(checks_messages::isolation_contactor_feedback_missing_when_on);
    }
    if(!EstopIO->inputs.main_contactor_ext && EstopIO->outputs.main_contactor){
        logs->log_persistent_active(checks_messages::main_contactor_feedback_missing_when_on);
    }
    else{
        logs->log_persistent_inactive(checks_messages::main_contactor_feedback_missing_when_on);
    }
    if(!EstopIO->inputs.precharge_contactor_ext && EstopIO->outputs.precharge_contactor){
        logs->log_persistent_active(checks_messages::precharge_contactor_feedback_missing_when_on);
    }
    else{
        logs->log_persistent_inactive(checks_messages::precharge_contactor_feedback_missing_when_on);
    }
}
...
void Mode::run_logic(void){
    ...
    if(!ext_chain_ok){
        ...
        if((*comm_vars)->enable_power && compare_aux_inputs((*comm_vars)->enable_required_inputs)){
            ...
            if(EstopIO->inputs.isolation_contactor_ext && EstopIO->inputs.main_contactor_ext){ // isolation contactor powered and closed
                ...
                if(EstopIO->inputs.main_contactor_ext && EstopIO->inputs.main_contactor_int){ // main contactor powered and closed
                    ...
                    if(EstopIO->inputs.precharge_contactor_ext && EstopIO->inputs.precharge_contactor_int){ // precharge contactor powered and closed
                        if(*micros > precharge_time){ // precharge time has passed
                            desired_contactor_state[1] = 1; // enable main contactor
                        }
                    }
                    else{
                        precharge_time = *micros + (*comm_vars)->precharge_delay*1000; // set precharge time to now + delay
                    }
                }   
            }
        }
...
void Mode::run_delays(void){
    ...
    if(EstopIO->outputs.isolation_contactor != desired_contactor_state[0]){
        if(desired_contactor_state[0] == 1){
            if((*micros - last_contactor_on_time[0]) > CONTACTOR_MINIMUM_OFF_TIME){
    ...
    if(EstopIO->outputs.main_contactor != desired_contactor_state[1]){
        if(desired_contactor_state[1] == 1){
            if((*micros - last_contactor_on_time[1]) > CONTACTOR_MINIMUM_OFF_TIME){
    ...
    if(EstopIO->outputs.precharge_contactor != desired_contactor_state[2]){
        if(desired_contactor_state[2] == 1){
            if((*micros - last_contactor_on_time[2]) > CONTACTOR_MINIMUM_OFF_TIME){
    ...
    if(EstopIO->outputs.brake_1 != desired_brake_state[0]){
        if(desired_brake_state[0] == 1){
            if((*micros - last_brake_on_time[0]) > BRAKE_MINIMUM_OFF_TIME){
    ...
    if(EstopIO->outputs.brake_2 != desired_brake_state[1]){
        if(desired_brake_state[1] == 1){
            if((*micros - last_brake_on_time[1]) > BRAKE_MINIMUM_OFF_TIME){
    ...
    if(EstopIO->outputs.brake_3 != desired_brake_state[2]){
        if(desired_brake_state[2] == 1){
            if((*micros - last_brake_on_time[2]) > BRAKE_MINIMUM_OFF_TIME){
    ...
    if(EstopIO->outputs.brake_4 != desired_brake_state[3]){
        if(desired_brake_state[3] == 1){
            if((*micros - last_brake_on_time[3]) > BRAKE_MINIMUM_OFF_TIME){
    ...
    if(EstopIO->outputs.brake_5 != desired_brake_state[4]){
        if(desired_brake_state[4] == 1){
            if((*micros - last_brake_on_time[4]) > BRAKE_MINIMUM_OFF_TIME){
    ...
    if(EstopIO->outputs.brake_6 != desired_brake_state[5]){
        if(desired_brake_state[5] == 1){
            if((*micros - last_brake_on_time[5]) > BRAKE_MINIMUM_OFF_TIME){
